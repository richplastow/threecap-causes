...
const tweenDefs = [
    ...
    { // camera position’s altitude
      ...
      , onReset:    function (def) {
            for (let i=0; i<scene.titles.length; i++)
                scene.titles[i].visible = false
            for (let i=0; i<scene.cutouts.length; i++)
                scene.cutouts[i].visible = false
            for (let i=0; i<scene.vidscreens.length; i++)
                scene.vidscreens[i].visible = false
